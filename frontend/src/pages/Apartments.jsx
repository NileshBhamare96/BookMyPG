  ...
  return (
    <>
      <div className="container mt-5" style={{backgroundColor:"aliceblue"}}>
        {role === 'Owner' ? (
          <Link to="/addnew" className="btn btn-primary btn-sm float-end">Add new Apartment</Link>
        ) : ""}
          ...
          <tbody>
            {paginatedData.map(x => (
              <tr key={x.id}>
                ...
                <td>
                  {role === 'Admin' ? (
                    <button onClick={() => deleteApartment(x.id)} className="btn btn-outline-danger btn-sm float-end">Delete Apartment</button>
                  ) : (
