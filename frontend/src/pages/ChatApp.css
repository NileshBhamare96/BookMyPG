...
const ChatApp = () => {
  ...
  return (
    <div className={`chat-container ${isChatOpen ? "open" : "closed"}`}>
      ...
      <div className="message-list" ref={inputTextRef}>
        {messages.map((inputText, index) => (
          <div
            key={index}
            className={`inputText ${
              inputText.type === "bot" ? "bot-message" : "user-message"
            }`}
          >
            <div className="message-text">{inputText.text}</div>
          </div>
        ))}
      </div>
      <div className="message-list" ref={messageListRef}>
